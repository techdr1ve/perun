...
//Kill Stats Query (button is in 2nd query below)
				...
				WHERE a.`pe_LogEvent_militarybranch` = 'MARINES' AND b.pe_dataplayers_id=?	
...
echo '<br><br><br><h3 style="margin:5px">Overall Standings by Military Branch: Marines <img src="red.png" alt="Paris" width="40" height="13"></h3>';
...
WHERE (a.`pe_LogEvent_militarybranch` = 'MARINES' AND a.`pe_LogEvent_militarybranch` NOT IN('','N/A'))
GROUP BY a.pe_LogEvent_pilotid
ORDER BY 
COUNT(a.`pe_LogEvent_kills_unarmed`)+
((COUNT(a.`pe_LogEvent_kills_infantry`)+COUNT(a.`pe_LogEvent_kills_artillery`))*2)+
...
//TABLE STARTS BELOW
	...
	echo "<tr class='table_header'><th>Place</th><th>Pilot</th><th>Flight Hours [(+1/10min)*%]</th><th>A2A Kills (+4)</th><th>A2G Kills (Up)</th><th>Sorties</th><th>Landing%</th><th>Ejections (+1)</th><th>Crashes (-1)</th><th>Deaths (-3)</th><th>Bomb Self (-1)</th><th>PVP Kills (+5)</th><th>Team Kills (-10)</th><th>Total</th><th><a href='hardcorestats_rankreq_usa.php'>Rank [Marines]</a></th><th><a href='hardcorestats_medalsreq_usa.php'>Medal Requirements</a></th></th></tr>"; // First Code							
	while($row = mysqli_fetch_array($result))
	{
		echo "<tr class='header'>";
			...
			elseif($row['rank_officer_usa_ma']=="[GAF] General of the Air Force (O-11)")
			{
			$rankshort = $GAF;
