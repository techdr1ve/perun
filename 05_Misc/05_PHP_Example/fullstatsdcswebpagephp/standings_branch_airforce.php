...
//Kill Stats Query (button is in 2nd query below)
				...
				WHERE a.`pe_LogEvent_militarybranch` = 'Air Force' AND b.pe_dataplayers_id=?	
...
echo '<br><br><br><h3 style="margin:5px">Overall Standings by Military Branch: Air Force <img src="lightblue.png" alt="Paris" width="40" height="13"></h3>';
...
WHERE (a.`pe_LogEvent_militarybranch` = 'AIR FORCE' AND a.`pe_LogEvent_militarybranch` NOT IN('','N/A'))
GROUP BY a.pe_LogEvent_pilotid
ORDER BY 
COUNT(a.`pe_LogEvent_kills_unarmed`)+
((COUNT(a.`pe_LogEvent_kills_infantry`)+COUNT(a.`pe_LogEvent_kills_artillery`))*2)+
...
//TABLE STARTS BELOW
	...
	echo "<tr class='table_header'><th>Place</th><th>Pilot</th><th>Flight Hours [(+1/10min)*%]</th><th>A2A Kills (+4)</th><th>A2G Kills (Up)</th><th>Sorties</th><th>Landing%</th><th>Ejections (+1)</th><th>Crashes (-1)</th><th>Deaths (-3)</th><th>Bomb Self (-1)</th><th>PVP Kills (+5)</th><th>Team Kills (-10)</th><th>Total</th><th><a href='hardcorestats_rankreq_usa.php'>Rank [Air Force]</a></th><th><a href='hardcorestats_medalsreq_usa.php'>Medal Requirements</a></th></th></tr>"; // First Code							
	while($row = mysqli_fetch_array($result))
	{
		echo "<tr class='header'>";
...
/* 			if($row['rank_officer_usa_nato']=="[2ndLt] Second Lieutenant (O-1)")
			...
			elseif($row['rank_officer_usa_af']=="[GAF] General of the Air Force (O-11)")
			{
			echo "<td style='text-align: center'>" . "<img src='./GA_t.png' height='60px' width='60px'/>" . "<br>General<br>of the<br>Air Force<br>(O-11)" . "<br><br>" . str_repeat("<a href='https://en.wikipedia.org/wiki/Purple_Heart' target='_blank' rel='noopener noreferrer'><img src='./purpleheart.png' class='thumbnail3' height='40px' width='21px'/></a>", floor($row['medals_usa_ph'])) . "</td>";	
			}			
			else
			{
			echo "<td style='text-align: center'>" . "<img src='./nothing.png' height='1px' width='1px'/>" . "</td>";	
