...
((SUM(a.`ps_farp_landings`)+SUM(a.`ps_airfield_landings`)+SUM(a.`ps_ship_landings`)+SUM(a.`ps_other_landings`))/(SUM(a.`ps_farp_takeoffs`)+SUM(a.`ps_airfield_takeoffs`)+SUM(a.`ps_ship_takeoffs`)+SUM(a.`ps_other_takeoffs`))*100) AS total_takeoff_landing_percentage, CAST('total_takeoff_landing_percentage' AS decimal(3,2)) AS test,
 ...
 (SUM(a.`ps_kills_armor`)+SUM(a.`ps_kills_unarmed`)+SUM(a.`ps_kills_ships`)+SUM(a.`ps_kills_artillery`)+SUM(a.`ps_kills_air_defense`)+SUM(a.`ps_kills_fortification`)+SUM(a.`ps_kills_infantry`)) AS total_a2gkills, (SUM(a.`ps_kills_planes`)+SUM(a.`ps_kills_helicopters`)) AS total_a2akills, 
 c.`pe_DataMissionHashes_id`, 
 c.`pe_DataMissionHashes_hash`, 
 SUBSTRING(c.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(c.`pe_DataMissionHashes_hash`) - 26) AS value_mission,
 SUBSTRING(c.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(c.`pe_DataMissionHashes_hash`) - 30) AS value_campaign 
 
 FROM `pe_LogStats` AS a 
 INNER JOIN `pe_DataPlayers` AS b ON a.pe_LogStats_playerid = b.pe_DataPlayers_id 
 INNER JOIN `pe_DataMissionHashes` AS c ON a.`pe_LogStats_missionhash_id` = c.`pe_DataMissionHashes_id` 
 
 WHERE SUBSTRING(c.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(c.`pe_DataMissionHashes_hash`) - 26) LIKE '%DSMC%' 
 ...
 
 {
	...
	echo "<tr class='table_header'><th style='text-align: center' colspan='3'>Main Events</th><th style='text-align: center' colspan='5'>Life Events</th><th style='text-align: center' colspan='2'>PVP Events</th><th style='text-align: center' colspan='2'>A2A Kills</th><th style='text-align: center' colspan='8'>A2G Kills</th></tr>"; // First Code	
	echo "<tr class='table_header'><th>Campaign</th><th>A2A Kills</th><th>A2G Kills</th><th>Sorties</th><th>Landing%</th><th>Deaths</th><th>Ejections</th><th>Crashes</th><th>PVP Kills</th><th>Team Kills</th><th>Planes</th><th>Helos</th><th>Tanks|APCs|LUVs</th><th>UnArmored</th><th>Air Defense</th><th>Artillery</th><th>Infantry</th><th>Ships</th><th>Structures</th><th>Hits|Aesthetic Statics</th></tr>"; // First Code					
