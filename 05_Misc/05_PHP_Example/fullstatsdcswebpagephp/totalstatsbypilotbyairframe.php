...
(SUM(a.`ps_kills_armor`)+SUM(a.`ps_kills_unarmed`)+SUM(a.`ps_kills_ships`)+SUM(a.`ps_kills_artillery`)+SUM(a.`ps_kills_fortification`)+SUM(a.`ps_kills_infantry`)) AS total_a2gkills,
 (SUM(a.`ps_kills_planes`)+SUM(a.`ps_kills_helicopters`)) AS total_a2akills,
 
 (SUM(a.`ps_kills_armor`)+SUM(a.`ps_kills_unarmed`)+SUM(a.`ps_kills_ships`)+SUM(a.`ps_kills_artillery`)+SUM(a.`ps_kills_fortification`)+SUM(a.`ps_kills_infantry`))+(SUM(a.`ps_kills_planes`)+SUM(a.`ps_kills_helicopters`)) AS total_kills,
 
 
 

 c.`pe_DataTypes_name`, d.`pe_DataMissionHashes_id`, d.`pe_DataMissionHashes_hash`, SUBSTRING(d.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(d.`pe_DataMissionHashes_hash`) - 26) AS value_mission FROM `pe_LogStats` AS a INNER JOIN `pe_DataPlayers` AS b ON a.pe_LogStats_playerid = b.pe_DataPlayers_id INNER JOIN `pe_DataTypes` AS c ON a.pe_LogStats_typeid = c.pe_DataTypes_id INNER JOIN `pe_DataMissionHashes` AS d ON a.`pe_LogStats_missionhash_id` = d.`pe_DataMissionHashes_id` WHERE((SUBSTRING(d.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(d.`pe_DataMissionHashes_hash`) - 26) LIKE '%_%') AND c.`pe_DataTypes_name` NOT IN ('?_-1','instructor', 'observer', 'forward_observer')) GROUP BY c.`pe_DataTypes_id`, b.`pe_DataPlayers_lastname` ORDER BY c.`pe_DataTypes_name`, b.`pe_DataPlayers_lastname` ")) {
...
ON a.pe_LogEvent_datetime = g.pe_finalflightduration_takeoffdatetime 
 ...
 GROUP BY a.`pe_LogEvent_airframe`, a.`pe_LogEvent_pilotname` 
 ORDER BY a.`pe_LogEvent_airframe`, a.`pe_LogEvent_pilotname` 
...
/* <div class="dropdown">
  ...
  <div id="myDropdown" class="dropdown-content">
	...
	echo "<input type='text' id='myInput' onkeyup='myFunction()' placeholder='Filter by pilot names...' title='Type in a name'>";	// searchable entry
	echo "<table class='table_stats' id='myTable' >";
	echo "<tr class='table_header'><th style='text-align: center' colspan='5'>Aviator Record Summary</th><th style='text-align: center' colspan='6'>Life Events</th><th style='text-align: center' colspan='2'>PVP Events</th><th style='text-align: center' colspan='2'>A2A Kills</th><th style='text-align: center' colspan='10'>A2G Kills</th></tr>"; // First Code	
	echo "<tr class='table_header'><th>Pilot Name</th><th>Airframe</th><th>Flight Hours</th><th>A2A Kills</th><th>A2G Kills</th><th>Sorties</th><th>Landing%</th><th>Deaths</th><th>Ejections</th><th>Crashes</th><th>Bomb Self</th><th>PVP Kills</th><th>Team Kills</th><th>Planes</th><th>Helos</th><th>Tanks|APCs|LUVs</th><th>UnArmored</th><th>Air Defense</th><th>Artillery</th><th>TEL Missile</th><th>Infantry</th><th>Ships</th><th>Warehouses</th><th>Structures</th><th>Hits|Statics</th></tr>"; // First Code					
	while($row = mysqli_fetch_array($result))
	{
		echo "<tr>";
