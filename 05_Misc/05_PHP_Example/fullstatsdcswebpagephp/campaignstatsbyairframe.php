...
echo "<h3 style='margin:5px'>CAMPAIGN Statistics by AIRFRAME WITH KEY --> _### </h3>";
...
((SUM(a.`ps_farp_landings`)+SUM(a.`ps_airfield_landings`)+SUM(a.`ps_ship_landings`)+SUM(a.`ps_other_landings`))/(SUM(a.`ps_farp_takeoffs`)+SUM(a.`ps_airfield_takeoffs`)+SUM(a.`ps_ship_takeoffs`)+SUM(a.`ps_other_takeoffs`))*100) AS total_takeoff_landing_percentage, CAST('total_takeoff_landing_percentage' AS decimal(3,2)) AS test,
 ...
 b.`pe_DataPlayers_lastname`, (SUM(a.`ps_kills_armor`)+SUM(a.`ps_kills_unarmed`)+SUM(a.`ps_kills_ships`)+SUM(a.`ps_kills_artillery`)+SUM(a.`ps_kills_air_defense`)+SUM(a.`ps_kills_fortification`)+SUM(a.`ps_kills_infantry`)) AS total_a2gkills, (SUM(a.`ps_kills_planes`)+SUM(a.`ps_kills_helicopters`)) AS total_a2akills, c.`pe_DataTypes_name`, d.`pe_DataMissionHashes_id`, d.`pe_DataMissionHashes_hash`, SUBSTRING(d.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(d.`pe_DataMissionHashes_hash`) - 26) AS value_mission, SUBSTRING(d.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(d.`pe_DataMissionHashes_hash`) - 30) AS value_campaign FROM `pe_LogStats` AS a INNER JOIN `pe_DataPlayers` AS b ON a.pe_LogStats_playerid = b.pe_DataPlayers_id INNER JOIN `pe_DataTypes` AS c ON a.pe_LogStats_typeid = c.pe_DataTypes_id INNER JOIN `pe_DataMissionHashes` AS d ON a.`pe_LogStats_missionhash_id` = d.`pe_DataMissionHashes_id` WHERE((SUBSTRING(d.`pe_DataMissionHashes_hash`, 1, CHAR_LENGTH(d.`pe_DataMissionHashes_hash`) - 26) LIKE '%DSMC%') AND c.`pe_DataTypes_name` NOT IN ('?_-1','instructor', 'observer', 'forward_observer')) GROUP BY value_campaign, c.`pe_DataTypes_name` ")) {
	...
	echo "<input type='text' id='myInput' onkeyup='myFunction()' placeholder='Filter by campaign name...' title='Type in a name'>";	// searchable entry
